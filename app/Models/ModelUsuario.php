...
class ModelUsuario extends Model
{
    ...
    {
        $query = "SELECT * FROM dblink('dbname=postgres user=postgres password=santiago10.', 'SELECT id,name,email,rol,campanas,activo,campana_activa,fecha_ingreso_campana FROM users') AS t(id INT, name TEXT,email TEXT,rol TEXT,campanas TEXT,activo TEXT, campana_activa TEXT, fecha_ingreso_campana TEXT)";
        $resultados = DB::select($query);
        return $resultados;  
    }
    public static function getUsuariosId($id)
    {
        $query = "SELECT * FROM dblink('dbname=postgres user=postgres password=santiago10.', 'SELECT id,name,email,rol,campanas,activo,estado,campana_activa,fecha_ingreso_campana FROM users where id = $id') AS t(id INT, name TEXT,email TEXT,rol TEXT,campanas TEXT,activo TEXT,estado TEXT, campana_activa TEXT, fecha_ingreso_campana TEXT)";
        $resultados = DB::select($query);
        return $resultados;  
    }
